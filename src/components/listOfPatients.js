...
//
function ListOfPatients({ loading, error, clients = [], detailsBasePath = '/clientDetails' }) {
	const formatLastSync = (client) => {// Handle missing or invalid lastSync
		if (!client?.lastSync) return client?.lastSyncDisplay || 'N/A';
		const syncDate = new Date(client.lastSync);// Handle invalid date
        ...
        // For older syncs, show the date
		return client?.lastSyncDisplay || syncDate.toLocaleDateString();
	};

    // show client list with name, age, last sync, status, and link to details page
	return (
		<section className="mt-8 rounded-2xl border border-gray-300 bg-white p-6 shadow-[0_2px_6px_rgba(0,0,0,0.08)]">
			...
			<div className="mt-4 rounded-xl border border-gray-300 bg-white p-3">
				...
				) : error ? ( // Display error message if there's an error fetching clients
					<div className="p-4 text-sm text-red-600">{error}</div>
				) : clients.length === 0 ? (// Handle case where there are no clients to display
						...
						{clients.map((client) => (
							...
							>
								<div className="flex min-w-0 items-center gap-3">
									<div className="h-8 w-8 shrink-0 rounded-full bg-gray-500 flex items-center justify-center text-[11px] font-semibold text-white">
                                        {/* the dashboard icon color cannot be changed*/}
                                        <img src = "/Dashboard_Icon.svg" alt="Dashboard Icon" className="h-4 w-4"/>
									</div>
								...
								<div className="flex items-center gap-3 self-end sm:self-auto">
									...
									<Link
										href={`${detailsBasePath}/${client.id}`}
